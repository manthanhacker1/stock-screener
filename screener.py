...
def get_supertrend(df, period=10, multiplier=3):
    ...
    for i in range(1, len(df)):
        if basic_upper.iloc[i] < upperband.iloc[i-1] or close.iloc[i-1] > upperband.iloc[i-1]:
            upperband.iloc[i] = basic_upper.iloc[i]
        else:
            upperband.iloc[i] = upperband.iloc[i-1]

        if basic_lower.iloc[i] > lowerband.iloc[i-1] or close.iloc[i-1] < lowerband.iloc[i-1]:
...
def analyze_stock(ticker):
    try:
        ...
        debt_to_equity = debt / equity if equity else 999
        roce = info.get("returnOnAssets", 0) * 100

        if market_cap <= 1000 or debt_to_equity >= 1 or roce <= 12:
            return None

        if "Diluted EPS" in financials.index and len(financials.columns) >= 5:
            eps = financials.loc["Diluted EPS"]
            eps_q0, eps_q1, eps_q4 = eps.iloc[0], eps.iloc[1], eps.iloc[4]
            if not (eps_q0 > eps_q1 and eps_q0 > eps_q4 and eps_q0 > 0):
                return None
        else:
            return None
        ...
        if st_daily_buy and st_weekly_buy and st_monthly_buy and close_price > st_daily_val:
            status = "STRONG BUY ✅"
